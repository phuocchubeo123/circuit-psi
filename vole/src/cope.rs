...
impl Cope {
    ...
    pub fn extend_sender<IO: CommunicationChannel>(&mut self, io: &mut IO, comm: &mut u64) -> FE {
        ...
        if let Some(prgs) = &mut self.prg_g0 {
            ...
            for (i, prg) in prgs.iter_mut().enumerate() {
                ...
                crate::fourq_field::random_fourq_elements_from_prg(prg, &mut [w[i]]);
            }
        }
    ...
    pub fn extend_receiver<IO: CommunicationChannel>(&mut self, io: &mut IO, u: FE, comm: &mut u64) -> FE {
        ...
        if let (Some(prgs_g0), Some(prgs_g1)) = (&mut self.prg_g0, &mut self.prg_g1) {
            for i in 0..self.m {
                crate::fourq_field::random_fourq_elements_from_prg(&mut prgs_g0[i], &mut [w0[i]]);
                crate::fourq_field::random_fourq_elements_from_prg(&mut prgs_g1[i], &mut [w1[i]]);
