...
fn main() -> Result<()> {
    ...
    // Must match shuffler's deterministic protocol RNG seed.
    ...
    let mut swanky = connect_swanky_with_retry(SWANKY_ADDR).context("connect swanky channel")?;
    let mut tcp = connect_with_retry(TCP_ADDR).context("connect tcp channel")?;
    ...
    let mut k1_prime_mul_vole_sender = BufferedVoleSender::<FourQVoleMac>::init(
        &mut swanky,
        &mut vole_rng,
