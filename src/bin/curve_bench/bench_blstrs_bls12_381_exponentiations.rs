...
fn main() -> Result<()> {
    ...
    println!(
        "Preparing {} blstrs BLS12-381 G1 exponents with seed {}...",
        n, seed
    );
    ...
    black_box(products);
    println!(
        "computed {} blstrs BLS12-381 G1 exponentiations in {:?}",
        n, elapsed
    );
    println!(
...
fn random_scalars(n: usize, seed: u64) -> Vec<Scalar> {
    ...
    while scalars.len() < n {
        ...
        let maybe_scalar: Option<Scalar> = Scalar::from_bytes_le(&bytes).into();
...
fn parse_args() -> Result<(usize, u64)> {
    ...
    if let Some(extra) = args.next() {
        return Err(anyhow::anyhow!(
            "Unexpected extra argument '{}'. Usage: bench_blstrs_bls12_381_exponentiations [count] [seed]",
            extra
        ));
    }
