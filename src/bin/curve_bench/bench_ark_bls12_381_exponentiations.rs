...
fn main() -> Result<()> {
    ...
    println!(
        "Preparing {} ark-bls12-381 G1 exponents with seed {}...",
        n, seed
    );
    ...
    black_box(products);
    println!(
        "computed {} ark-bls12-381 G1 exponentiations in {:?}",
        n, elapsed
    );
    println!(
...
fn parse_args() -> Result<(usize, u64)> {
    ...
    if let Some(extra) = args.next() {
        return Err(anyhow::anyhow!(
            "Unexpected extra argument '{}'. Usage: bench_ark_bls12_381_exponentiations [count] [seed]",
            extra
        ));
    }
