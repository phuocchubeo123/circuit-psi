...
impl TwoSideInputer {
    pub fn new(delta0: FE, k0: FE) -> Self {
        Self {
            inputer: Inputer::new(delta0, k0),
            shuffler: Shuffler::new(delta0, k0),
        }
    }
    ...
    pub fn run_full_two_side_shuffled_oprf<RNG: Rng>(
        ...
        // Receive the peer's shuffled OPRF proof first (inputer role), then send ours
        // (shuffler role). The mirrored two_side_shuffle_shuffler role does the opposite.
