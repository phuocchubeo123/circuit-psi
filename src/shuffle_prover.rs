use crate::{
    bedoza::{
        batch_multiply,
        bedoza_sender::{send_open_shares, BeDOZaSender},
        comm_util::{receive_fe_vec, send_fe, send_fe_vec},
        defines::{random_fe_vec, random_fe_vec_from_rng, FE},
        open_values_receive, open_values_send, receive_share_values, share_values, take_vec_prod,
        BeDOZa, BeDOZaTriple,
    },
    group::{send_group_elements, Group},
...
pub fn shuffle_prover_step1(
    ...
    let r_times_k_plus_x_shares = batch_multiply(
        &k_plus_x_shares,
        prepared_randomness,
...
pub fn shuffle_prover_share_s(
    ...
    prepared_s_prover_share: &BeDOZaSender,
    ...
    let s_inv_share = receive_share_values(&[*prepared_s_inv_share], channel)
        .map_err(|e| { anyhow!("Failed to receive shared inverse mask s from verifier: {}", e)})?[0];

    let s_consistency_proof = batch_multiply(
        &[s_share],
        &[s_inv_share],
    ...
    // Receive sender-only BeDOZa share for s (counterpart of share_values_sender on verifier side).
    send_open_shares(&[*prepared_s_prover_share], channel)
        .map_err(|e| anyhow!("Failed to send opened prepared sender share for s: {}", e))?;
    let masked_vals = receive_fe_vec(channel)
        .map_err(|e| anyhow!("Failed to receive masked sender-only value for s: {}", e))?;
    ensure!(
        masked_vals.len() == 1,
        "Expected exactly one masked sender-only value for s, got {}",
        masked_vals.len()
    );
    let s_prover_sender_share = *prepared_s_prover_share + masked_vals[0];

    // Verify sender-share consistency by checking opened value of
    // (s_sender_from_BeDOZa - s_sender_from_sender_only) equals zero.
    let sender_consistency_local = BeDOZa::new(
        *s_share.bedoza_sender() - s_prover_sender_share,
        *s_share.bedoza_receiver(),
    );
    let opened_sender_consistency = open_values_receive(&[sender_consistency_local], channel)
        .map_err(|e| anyhow!("Failed to open sender consistency proof for mask s: {}", e))?[0];
    ensure!(
        opened_sender_consistency == FE::zero(),
        "Sender share consistency check failed: expected 0, got {:?}",
        opened_sender_consistency
    );
