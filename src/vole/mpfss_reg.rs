use crate::{
    ...
    pre_ot::OTPre,
    spfss_receiver::SpfssRecverFp,
    spfss_sender::SpfssSenderFp,
...
impl MpfssReg {
    ...
    pub fn mpfss_sender(
        ...
        for i in 0..self.tree_n {
            ...
            sparse_vector[i * self.leave_n..(i + 1) * self.leave_n]
                .copy_from_slice(&self.ggm_tree[i]);
    ...
    pub fn mpfss_receiver(
        &mut self,
        io: &mut SwankyChannel,
        ot: &mut OTPre<FE_LIMBS>,
        triple_y: &[FE],
        triple_z: &[FE],
        sparse_vector_y: &mut [FE],
        sparse_vector_z: &mut [FE],
        comm: &mut u64,
    ) {
        // triple_y_recv = triple_y_send + delta * triple_z
        ...
        for i in 0..self.tree_n {
            ...
            sparse_vector_y[i * self.leave_n..(i + 1) * self.leave_n]
                .copy_from_slice(&self.ggm_tree[i]);
            ...
            if self.is_malicious {
                receiver.consistency_check_msg_gen(
                    &mut self.check_chialpha_buf[i],
                    &mut self.check_vw_buf[i],
                    seeds[i],
                );
            }
        }
    ...
    pub fn seed_expand(
        ...
        random_fourq_elements_from_prg(&mut prg2, seed);
    }
}
